...
### 2.2 ขั้นตอนที่ 0: Preflight Health Check
ตรวจสอบความพร้อมของระบบพื้นฐานทั้งหมด (AD, M365, PaperCut, Redis) หากเชื่อมต่อไม่ได้ จะยกเลิกงาน (Cancel) ทันทีเพื่อป้องกันความผิดพลาดระหว่างทาง

### 2.3 ขั้นตอนที่ 1: AD Creation Task (`run_ad_creation_task`)
...
### 2.4 ขั้นตอนที่ 2: PaperCut Sync Task (`run_papercut_task`)
...
### 2.5 ขั้นตอนที่ 3: Microsoft 365 License Task (`run_m365_license_task`)
* **กระบวนการทำงานมี 3 ขั้นตอนย่อย (Sub-sequence):** (รายละเอียดมาตรฐานการบันทึกและแสดงผลแบบละเอียดดูได้ที่ [sequence.md](file:///c:/Users/wajeepradit.p/git/profile_automate/worker/sequence.md))
  1. **สเต็ปที่ 1: การรอคอยเริ่มต้น (Entra Cloud Sync / Azure AD Connect)**: ในโปรดักชันระบบจะพักงานไว้ 5 นาทีเริ่มต้น
  2. **สเต็ปที่ 2: Pre-check & Exponential Retry**: ตรวจสอบว่าบัญชีมีอยู่ใน Azure AD แล้วหรือยังผ่าน Graph API หากยังไม่มี จะ re-enqueue ตัวเองและรอเพิ่มด้วย Exponential Backoff สูงสุด 6 ครั้ง (~30 นาที)
  3. **สเต็ปที่ 3: SKU Resolution & Assignment**: ดำเนินการแปลงชื่อ SKU เป็น GUID แล้วผูกลิขสิทธิ์ (เช่น E3, EMS) ไปยังบัญชีเป้าหมายผ่าน Graph API
...
### 2.6 ขั้นตอนที่ 4: Email Notification Task (`run_send_email_task`)
